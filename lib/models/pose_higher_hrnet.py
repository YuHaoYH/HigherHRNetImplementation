...
class BasicBlock(nn.Module):  # 两种基本单元之一
    expansion = 1
...
class Bottleneck(nn.Module):  # 两种基本单元之一
    expansion = 4
    ...
    def forward(self, x):
        ...
        # 这里的downs ample的作用是希望输入原图x与conv3输出的图维度相同，方便两种特征图进行相加，保留更多的信息（你要是看不懂这句话，就去先简单了解一下残差结构）
        # 如果x与conv3输出图维度本来就相同，就意味着可以直接相加，那么downs ample会为空，自然就不会进行下面操作
...
class HighResolutionModule(nn.Module):  # 高分辨率模块
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
        ...
        '''
        param：
        num_branches:当前stage分支平行子网络数目
        blocks:BasicBlock或者BottleNeckBlock
        num_blocks:block的数目
        ...
        # 为每个分支构建网络
        # 当stage=2,3,4时，num_branches分别是：2，3，4，表示每个stage平行网络的数目
        # 当stage=2,3,4时，num_blocks分别是：[4,4], [4,4,4], [4,4,4,4]
    ...
    def _make_branches(self, num_branches, block, num_blocks, num_channels):
        ...
        # stage=2时, self._make_one_branch(0, BASICBLOCK, [4,4], [32,64]) ,
        # self._make_one_branch(1, BASICBLOCK, [4,4], [32,64])
...
def get_pose_net(cfg, is_train, **kwargs):
    ...
    # 利用PoseHighResolutionNet来构建整个模型
